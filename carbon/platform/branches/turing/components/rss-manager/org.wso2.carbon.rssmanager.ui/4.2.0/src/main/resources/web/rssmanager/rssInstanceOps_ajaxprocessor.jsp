...
<%
    ...
    if ("create".equals(flag)) {
        try {
            ...
            rssIns.getDataSourceConfig().setUrl(serverUrl);
            rssIns.getDataSourceConfig().setUsername(username);
            rssIns.getDataSourceConfig().setPassword(password);
            ...
            ctx.setRssInstanceName(rssIns.getName());
            //TODO properly set RSS environment name
            client.createRSSInstance(ctx, rssIns);
        ...
        try {
            RSSEnvironmentContext ctx = new RSSEnvironmentContext();
            ctx.setRssInstanceName(rssInstanceName);
            //TODO properly set RSS environment name
            client.dropRSSInstance(ctx);
            response.setContentType("text/xml; charset=UTF-8");
            // Set standard HTTP/1.1 no-cache headers.
            response.setHeader("Cache-Control",
                    "no-store, max-age=0, no-cache, must-revalidate");
        ...
        try {
            ...
            ctx.setRssInstanceName(rssIns.getName());
            //TODO properly set RSS environment name
            client.editRSSInstance(ctx, rssIns);
            response.setContentType("text/xml; charset=UTF-8");
            // Set standard HTTP/1.1 no-cache headers.
            response.setHeader("Cache-Control",
                    "no-store, max-age=0, no-cache, must-revalidate");
