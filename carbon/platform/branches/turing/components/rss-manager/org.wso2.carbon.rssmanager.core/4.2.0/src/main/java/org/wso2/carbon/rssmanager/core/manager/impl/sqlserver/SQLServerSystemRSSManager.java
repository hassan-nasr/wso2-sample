...
public class SQLServerSystemRSSManager extends SystemRSSManager {
    ...
    public SQLServerSystemRSSManager(Environment environment, RSSConfig config) {
    ...
    public Database createDatabase(Database database) throws RSSManagerException {
        ...
        try {
            ...
            this.getRSSDAO().getDatabaseDAO().addDatabase(getEnvironmentName(), database,
                    tenantId);
    ...
    public void editDatabaseUserPrivileges(DatabasePrivilegeSet privileges, DatabaseUser user,
        ...
        RSSInstance rssInstance;
        try {
            String rssInstanceName = this.getRSSDAO().getDatabaseUserDAO().resolveRSSInstanceByUser(
                    this.getEnvironmentName(), user.getRssInstanceName(), user.getType(),
                    user.getName(), tenantId);
            rssInstance = this.getEnvironment().getRSSInstance(rssInstanceName);
        ...
        try {
            ...
            //TODO Explain why this is done when the type is WSO2_RSS
            user.setRssInstanceName(rssInstance.getName());
    ...
    public void detachUserFromDatabase(UserDatabaseEntry entry) throws RSSManagerException {
        ...
        /* Initiating the distributed transaction */
        RSSInstance rssInstance =
                resolveRSSInstanceByDatabase(entry.getDatabaseName());
        if (rssInstance == null) {
