...
public final class RSSManagerFactoryLoader {
    ...
    public static RSSManagerFactory getRMFactory(String type, RSSConfig config,
        ...
        if (RSSManagerConstants.RSSManagerProviderTypes.
            ...
            return new MySQLRSSManagerFactory(environment, config);
        } else if (RSSManagerConstants.RSSManagerProviderTypes.
                RM_PROVIDER_TYPE_ORACLE.equals(type)) {
            return new OracleRSSManagerFactory(environment, config);
        } else if (RSSManagerConstants.RSSManagerProviderTypes.
                RM_PROVIDER_TYPE_SQLSERVER.equals(type)) {
            return new SQLServerRSSManagerFactory(environment, config);
        } else if (RSSManagerConstants.RSSManagerProviderTypes.
                RM_PROVIDER_TYPE_POSTGRES.equals(type)) {
            return new PostgresRSSManagerFactory(environment, config);
        } else if (RSSManagerConstants.RSSManagerProviderTypes.
                RM_PROVIDER_TYPE_H2.equals(type)) {
            return new H2RSSManagerFactory(environment, config);
        } else {
