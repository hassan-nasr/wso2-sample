...
public class PassiveSTSService {
	...
	public ResponseToken getResponse(RequestToken request) throws Exception {
		...
		if (IdentityPassiveSTSServiceComponent.getRealmService() == null) {
		...
		UserRealm realm = AnonymousSessionUtil.getRealmByUserName(
				IdentityPassiveSTSServiceComponent.getRegistryervice(),
				IdentityPassiveSTSServiceComponent.getRealmService(), request.getUserName());
		String tenantAwareUserName = MultitenantUtils.getTenantAwareUsername(request.getUserName());

		if (!realm.getUserStoreManager().isValidRememberMeToken(request.getUserName(),
				request.getPseudo())) {
			boolean isAuthenticated = realm.getUserStoreManager().authenticate(tenantAwareUserName,
					request.getPassword());

			if (!isAuthenticated) {
				return new ResponseToken();
			} else {
				realm.getUserStoreManager().addRememberMe(tenantAwareUserName, request.getPseudo());
				String remoteAddress = AuthenticationUtil.getRemoteAddress(MessageContext
						.getCurrentMessageContext());

				int tenantId = IdentityUtil.getTenantIdOFUser(request.getUserName());
				...
				CarbonAuthenticationUtil.onSuccessAdminLogin(session, request.getUserName(),
						tenantId, MultitenantUtils.getTenantDomain(request.getUserName()),
						remoteAddress);
			}
		}
