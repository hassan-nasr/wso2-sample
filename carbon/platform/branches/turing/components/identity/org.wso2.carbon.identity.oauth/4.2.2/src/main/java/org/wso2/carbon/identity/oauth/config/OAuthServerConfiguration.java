...
public class OAuthServerConfiguration {
    ...
    private String tokenPersistenceProcessorClassName = "org.wso2.carbon.identity.oauth.tokenprocessor.PlainTextProcessor";
	...
	private TokenPersistenceProcessor tokenPersistenceProcessor = null;
	...
	public TokenPersistenceProcessor getTokenPersistenceProcessor() throws IdentityOAuth2Exception {
        if(tokenPersistenceProcessor == null){
            synchronized (this) {
                if(tokenPersistenceProcessor == null) {
                    try {
                        Class clazz =
                        ...
                        tokenPersistenceProcessor = (TokenPersistenceProcessor) clazz.newInstance();
                        ...
                        String errorMsg =
                                "Error when instantiating the TokenPersistencePreprocessor : " +
                                        tokenPersistenceProcessorClassName + ". Defaulting to PlainTextProcessor";
        ...
        return tokenPersistenceProcessor;
	}
	...
	private void parseTokenPersistenceProcessorConfig(OMElement oauthConfigElem) {
		...
		if (preprocessorConfigElem != null && !preprocessorConfigElem.getText().trim().equals("")) {
			tokenPersistenceProcessorClassName = preprocessorConfigElem.getText().trim();
		}
	...
	private void parseSupportedGrantTypesConfig(OMElement oauthConfigElem) {
        ...
        if(log.isDebugEnabled()){
            for(Map.Entry entry : supportedGrantTypes.entrySet()){
	...
	private void parseSupportedResponseTypesConfig(OMElement oauthConfigElem) {
        ...
        if(log.isDebugEnabled()){
            for(Map.Entry entry : supportedResponseTypes.entrySet()){
