...
public class OpenIDHandler {
	...
	private void handleRequestFromLoginPage(HttpServletRequest req, HttpServletResponse resp) throws IdentityException {
		...
		try {
			...
			// if not found, do authenticate
			if (!isAuthenticated) {
                isAuthenticated = client.authenticateWithOpenID(claimedID,
                        req.getParameter(OpenIDConstants.RequestParameter.PASSWORD), session, req, resp,
                        isRemembered);
			}
			...
			if (isAuthenticated) {
				...
				if (client.isOpenIDUserApprovalBypassEnabled()) {
                    ...
                    if (client.getNewCookieValue() != null) {
                        OpenIDUtil.setCookie(OpenIDConstants.Cookie.OPENID_TOKEN, client.getNewCookieValue(),
					...
					if (rpInfo[0].equals("true")) { // approve always
                        ...
                        if (client.getNewCookieValue() != null) {
                            OpenIDUtil.setCookie(OpenIDConstants.Cookie.OPENID_TOKEN,
                                    client.getNewCookieValue(), client.getOpenIDSessionTimeout(), "/", "",
                                    true, resp);
                        }
