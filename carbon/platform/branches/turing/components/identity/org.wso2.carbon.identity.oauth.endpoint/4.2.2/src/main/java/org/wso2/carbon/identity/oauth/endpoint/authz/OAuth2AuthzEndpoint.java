...
public class OAuth2AuthzEndpoint {
    ...
    public Response authorize(@Context HttpServletRequest request) throws URISyntaxException {
        ...
        String sessionDataKeyFromLogin = EndpointUtil.getSafeText((String)request.getAttribute(OAuthConstants.SESSION_DATA_KEY));
        String sessionDataKeyFromConsent = EndpointUtil.getSafeText(request.getParameter(OAuthConstants.SESSION_DATA_KEY_CONSENT));
        CacheKey cacheKey;
	...
	private String doUserAuthz(HttpServletRequest request, String sessionDataKey,
        ...
        if (oauth2Params.getPrompt().contains(OIDC.Prompt.NONE)) {  // should not prompt for consent if approved always
