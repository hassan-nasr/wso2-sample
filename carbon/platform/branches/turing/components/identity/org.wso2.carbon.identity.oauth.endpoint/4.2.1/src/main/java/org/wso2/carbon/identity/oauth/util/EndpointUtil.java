...
public class EndpointUtil {
     ...
     * @return
     */
	public static String getErrorPageURL(HttpServletRequest req, OAuth2Parameters oauth2Params,
		...
		try {
			errorPageUrl += "?" + OAuthConstants.OAUTH_ERROR_CODE + "="
					...
					+ URLEncoder.encode(errorMessage, "UTF-8") + "&" + "application" + "=" + appName;
		} catch (UnsupportedEncodingException e) {
			// ignore
		}
		return errorPageUrl;
	...
	public static String getLoginPageURL(String sessionDataKey, boolean forceAuthenticate, boolean checkAuthentication) {
		...
		String queryParams = "";
		queryParams = "?" + OAuth2Constants.SESSION_DATA_KEY + "=" + sessionDataKey + "&type=oauth2"
        ...
        try {
            if(isOIDC){
                consentPage = CarbonUIUtil.getAdminConsoleURL("/") +
                    ...
                    consentPage +=
                            "?" + OpenIDConnectConstant.Session.OIDC_LOGGED_IN_USER + "=" +
                                    ...
                                    OAuth2Constants.SESSION_DATA_KEY + "=" +
                                    URLEncoder.encode(sessiondataKey, "UTF-8");
                ...
                consentPage +=
                    "?" + OpenIDConnectConstant.Session.OIDC_LOGGED_IN_USER + "=" +
                    URLEncoder.encode(loggedInUser, "UTF-8") + "&" +
                    "application" + "=" + URLEncoder.encode( EndpointUtil.getSafeText(params.getApplicationName()), "ISO-8859-1" ) + "&" +
                    OAuthConstants.SCOPE + "=" + URLEncoder.encode( EndpointUtil.getSafeText(EndpointUtil.getScope(params)), "ISO-8859-1" ) + "&" +
                    OAuth2Constants.SESSION_DATA_KEY + "=" + URLEncoder.encode(sessiondataKey, "UTF-8");
            }
        } catch (UnsupportedEncodingException e) {
            // ignore
        }
