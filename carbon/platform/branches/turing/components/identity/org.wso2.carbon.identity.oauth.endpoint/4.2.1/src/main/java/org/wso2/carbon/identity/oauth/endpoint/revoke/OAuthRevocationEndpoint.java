...
import org.wso2.carbon.identity.oauth.util.OAuthEndPointClientUtil;
    ...
    public Response revokeAccessToken(@Context HttpServletRequest request,
        ...
        try {
            ...
            if (request.getHeader(OAuthConstants.HTTP_REQ_HEADER_AUTHZ) != null) {
                try {
                    String[] clientCredentials = OAuthEndPointClientUtil
                            .extractCredentialsFromAuthzHeader(request
                                                                       .getHeader(OAuthConstants.HTTP_REQ_HEADER_AUTHZ));

                    // If the client has included client credentials as request parameters, then it
                    // is
                    ...
                    // keep track of the authentication mode : required for sending back auth.
                    // failure errors
                    basicAuthUsed = true;
            ...
            try {
                ...
                if (oauthRevokeResp.getErrorMsg() != null) {
                    ...
                    // if the client has used Basic Auth and if there is an auth failure, HTTP 401
                    // Status
                    // Code should be sent back to the client.
                    if (basicAuthUsed
                            && OAuth2ErrorCodes.INVALID_CLIENT.equals(oauthRevokeResp
                                    .getErrorCode())) {
                        return handleBasicAuthFailure(callback);
                    } else if (OAuth2ErrorCodes.UNAUTHORIZED_CLIENT.equals(oauthRevokeResp
                            .getErrorCode())) {
    ...
    private Response handleBasicAuthFailure(String callback)
        ...
        if(callback == null && callback.equals("")){
            ...
            return Response.status(response.getResponseStatus())
                    .header(OAuthConstants.HTTP_RESP_HEADER_AUTHENTICATE, OAuthEndPointClientUtil.getRealmInfo())
