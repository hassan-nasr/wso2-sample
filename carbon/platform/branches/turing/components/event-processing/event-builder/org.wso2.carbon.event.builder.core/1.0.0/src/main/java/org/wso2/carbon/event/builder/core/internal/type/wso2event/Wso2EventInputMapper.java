...
public class Wso2EventInputMapper implements InputMapper {
    ...
    public Wso2EventInputMapper(EventBuilderConfiguration eventBuilderConfiguration, StreamDefinition inputStreamDefinition) throws EventBuilderConfigurationException {
        validateInputStreamAttributes();
        this.eventBuilderConfiguration = eventBuilderConfiguration;

        String streamName = eventBuilderConfiguration.getInputStreamConfiguration().getInputEventAdaptorMessageConfiguration().getInputMessageProperties().get(EventBuilderConstants.ADAPTOR_MESSAGE_STREAM_NAME);
        String streamVersion = eventBuilderConfiguration.getInputStreamConfiguration().getInputEventAdaptorMessageConfiguration().getInputMessageProperties().get(EventBuilderConstants.ADAPTOR_MESSAGE_STREAM_VERSION);

        int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();

        if(streamName != null && streamVersion != null && (! streamName.isEmpty()) && (! streamVersion.isEmpty())){
            ...
            try {
                ...
                if(streamDefinition == null){
                    throw new EventBuilderStreamValidationException("Input stream definition not found in registry ",streamName+EventBuilderConstants.STREAM_NAME_VER_DELIMITER+streamVersion);
                }
