...
public final class CassandraEventAdaptorType extends AbstractOutputEventAdaptor {
    ...
    private ConcurrentHashMap<Integer, ConcurrentHashMap<String, EventAdaptorInfo>> tenantedCassandraClusterCache = new ConcurrentHashMap<Integer, ConcurrentHashMap<String, EventAdaptorInfo>>();
    ...
    public void publish(
        ...
        ConcurrentHashMap<String, EventAdaptorInfo> cassandraClusterCache = null;
        if (message instanceof Map) {
            try {

                cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                if (null == cassandraClusterCache) {
                    cassandraClusterCache = new ConcurrentHashMap<String, EventAdaptorInfo>();
                    if (null != tenantedCassandraClusterCache.putIfAbsent(tenantId, cassandraClusterCache)) {
                        cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                    }
                }

                EventAdaptorInfo eventAdaptorInfo = cassandraClusterCache.get(outputEventAdaptorConfiguration.getName());
                if (null == eventAdaptorInfo) {
                    ...
                    Cluster cluster = HFactory.createCluster(properties.get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_CLUSTER_NAME),
                    ...
                    if (null != cassandraClusterCache.putIfAbsent(outputEventAdaptorConfiguration.getName(), eventAdaptorInfo)) {
                        eventAdaptorInfo = cassandraClusterCache.get(outputEventAdaptorConfiguration.getName());
                    } else {
                ...
                if (cassandraClusterCache != null) {
                    cassandraClusterCache.remove(outputEventAdaptorConfiguration.getName());
                }
