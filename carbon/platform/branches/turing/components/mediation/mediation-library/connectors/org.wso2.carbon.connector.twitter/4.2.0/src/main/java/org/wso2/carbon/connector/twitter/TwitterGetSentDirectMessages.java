...
public class TwitterGetSentDirectMessages extends AbstractTwitterConnector {
	...
	try {
		...
		results = twitter.getSentDirectMessages(new Paging(Long.parseLong(page)));
	    } else {
     ...
     * @throws IOException
     */
    private OMElement performSearchMessages(List<DirectMessage> messages) throws XMLStreamException,
	    TwitterException, JSONException, IOException {
	OMElement resultElement = AXIOMUtil.stringToOM("<XMLPayload/>");

	for (DirectMessage place : messages) {
	    StringBuilder stringBuilder = new StringBuilder();
