...
public class CassandraServerDSComponent {
    ...
    private static final String CASSANDRA_SERVER_CONF_PATH = CarbonUtils.getEtcCarbonConfigDirPath() +
    ...
    protected void activate(ComponentContext componentContext) {
        try {
            ...
            if (isConfigurationExists()) {
                cassandraConfLocation = "file:" + CASSANDRA_SERVER_CONF_PATH;
            }
            System.setProperty("cassandra.config", cassandraConfLocation);
            System.setProperty("cassandra-foreground", "yes");
            int carbonPortOffset = readPortOffset();

            int cassandraRPCPort = readPortFromSystemVar(CASSANDRA_RPC_PORT, carbonPortOffset,
                    DEFAULT_CASSANDRA_RPC_PORT);
            System.setProperty("cassandra.rpc_port", Integer.toString(cassandraRPCPort));

            int cassandraStoragePort = readPortFromSystemVar(CASSANDRA_STORAGE_PORT,
            ...
            int cassandraSSLStoragePort = readPortFromSystemVar(CASSANDRA_SSL_STORAGE_PORT,
    ...
    private boolean isConfigurationExists() {
        if (!new File(CASSANDRA_SERVER_CONF_PATH).exists()) {
            log.info("There is no " + CASSANDRA_SERVER_CONF_PATH + ". Using the default " +
                    "configuration");
            return false;
        } else {
            return true;
