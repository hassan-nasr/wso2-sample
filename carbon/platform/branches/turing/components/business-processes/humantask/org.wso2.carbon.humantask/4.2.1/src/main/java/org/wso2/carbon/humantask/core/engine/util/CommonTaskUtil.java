            ...
            for (OrganizationalEntityDAO orgEntity : potentialOwners.getOrgEntities()) {
                if (OrganizationalEntityDAO.OrganizationalEntityType.GROUP.equals(
                    ...
                    if (hasMultipleOwners) {
                        ...
                        List<OrganizationalEntityDAO> orgEntities = potentialOwners.getOrgEntities();
                        // Org entities should be one since hasMultipleOwners is false doing check anyway
                        if(orgEntities.size() == 1) {

                            OrganizationalEntityDAO organizationalEntityDAO = orgEntities.get(0);
                            String name = organizationalEntityDAO.getName();
                            GenericHumanRoleDAO actualOwnerRole =
                                    pqe.createGHRForUsername(name, GenericHumanRole.GenericHumanRoleType.ACTUAL_OWNER);
