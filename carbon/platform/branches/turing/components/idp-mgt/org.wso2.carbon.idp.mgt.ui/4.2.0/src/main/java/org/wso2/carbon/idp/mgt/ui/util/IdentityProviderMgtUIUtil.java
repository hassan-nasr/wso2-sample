    ...
    public static TrustedIdPDTO getFormData(HttpServletRequest request) throws FileUploadException, Exception {
        if (ServletFileUpload.isMultipartContent(request)) {
            ...
            for (Object item : items) {
                ...
                } else if(name.equals("tokenEPAlias")){
                    FileItem fileItem = diskFileItem;
                    byte[] tokenEPAliasArray = fileItem.get();
                    if(tokenEPAliasArray != null && tokenEPAliasArray.length > 0){
                        tokenEPAlias = new String(tokenEPAliasArray);
                    }
                }
            }
            ...
            trustedIdPDTO.setTokenEPAlias(tokenEPAlias);
