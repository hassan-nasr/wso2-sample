...
public class ResourceSubmitter implements Runnable {
    ...
    public void run() {
        ...
        Date currentTime = indexingManager.getLastAccessTime();
        try {
            PrivilegedCarbonContext.startTenantFlow();
            try {
                currentTime = submitResource(currentTime, MultitenantConstants.SUPER_TENANT_ID,
            ...
            for (Tenant tenant : allTenants) {
                PrivilegedCarbonContext.startTenantFlow();
                try {
                    currentTime = submitResource(currentTime, tenant.getId(), tenant.getDomain());
                } finally {
                    PrivilegedCarbonContext.endTenantFlow();
                }
            }
