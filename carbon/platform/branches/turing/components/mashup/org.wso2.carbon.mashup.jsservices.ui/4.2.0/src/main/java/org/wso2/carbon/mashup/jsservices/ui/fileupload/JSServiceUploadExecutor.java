...
public class JSServiceUploadExecutor extends AbstractFileUploadExecutor {
	...
	public boolean execute(HttpServletRequest request,
		...
		String webContext = (String) request
				.getAttribute(CarbonConstants.WEB_CONTEXT);
		String serverURL = (String) request
				.getAttribute(CarbonConstants.SERVER_URL);
		...
		if (fileItemsMap == null || fileItemsMap.isEmpty()) {
			...
			CarbonUIMessage.sendCarbonUIMessage(message, CarbonUIMessage.ERROR,
					request, response, getContextRoot(request) + "/"
		...
		JSServiceUploadClient serviceUploaderClient = new JSServiceUploadClient(
				configurationContext, serverURL, cookie);
		MashupServiceAdminClient client = new MashupServiceAdminClient(cookie,
		...
		try {
			for (int i = 0; i < tempDataList.size(); i++) {
				...
				checkServiceFileExtensionValidity(fileName,
				...
				if (fileName.endsWith(".zip")) {
					...
					File zipFile = ((DeferredFileOutputStream) fileData
							.getFileItem().getOutputStream()).getFile();
					fileData.getFileItem().write(zipFile);
					tempFiles.add(zipFile);
					String[] files = archiveManipulator.check(zipFile
							.getAbsolutePath());
					boolean jsFile = false;
					boolean resourceFolder = false;
					// validates the structure of zip file. i.e. zip file shoud
					// contains only one
					// .js file in the root folder and no other file types, but
					// directories.
					for (String file : files) {
						if (file.indexOf("/") == -1) {
							String extension = file
									.substring(file.indexOf(".") + 1);
							String jsName = file.substring(0,
									(file.indexOf(".")));
							if ("js".equals(extension)) {
								if (jsFile) {
									message = getFileName(fileData
											.getFileItem().getName())
											+ " file should contain only "
											+ "one '.js' file in the root folder";
									log.error(message);
									CarbonUIMessage.sendCarbonUIMessage(
											message, CarbonUIMessage.ERROR,
											request, response,
								...
								message = getFileName(fileData.getFileItem()
										.getName())
										+ " file contains an invalid file in the root folder : "
										+ file;
								log.error(message);
					...
					if (!jsFile) {
						message = getFileName(fileData.getFileItem().getName())
								+ " file doesn't contain a '.js'"
								+ " file in the root folder";

						log.error(message);
						CarbonUIMessage.sendCarbonUIMessage(message,
								CarbonUIMessage.ERROR, request, response,
					...
					message = "File with extension " + fileName
							+ " is not supported!";
					CarbonUIMessage.sendCarbonUIMessage(message,
							CarbonUIMessage.ERROR, request, response,
							getContextRoot(request) + "/" + webContext
			...
			if (existingServices != null && existingServices.length != 0) {
				...
				for (int i = 0; i < existingServices.length; i++) {
					String existingService = existingServices[i]
							.substring(existingServices[i].indexOf("/") + 1);

					for (int j = 0; j < jsServiceDataList.size(); j++) {
						String fileName = jsServiceDataList.get(j)
								.getFileName();

						if (fileName.contains(".")) {
							zipCode = fileName.substring(fileName.indexOf("."));
						}
						if (fileName.replaceAll(zipCode, "").equals(
								existingService)) {
				...
				if (jsServiceDataList.size() > 0) {
					...
					for (int j = 0; j < jsServiceDataList.size(); j++) {
						if (j == 0) {
							serviceList.append(jsServiceDataList.get(j)
									.getFileName().replaceAll(zipCode, ""));
						} else {
							serviceList.append(","
									+ jsServiceDataList.get(j).getFileName()
											.replaceAll(zipCode, ""));
						}
					}

					JSServiceUploadData[] services = jsServiceDataList
							.toArray(new JSServiceUploadData[jsServiceDataList
									.size()]);
					serviceUploaderClient.uploadService(username, services);

					message = "Javascript services with the names-"
							+ serviceList.toString()
							+ " uploaded successfully,except services with the names- "
							+ stringBuilder.toString()
							+ " since they are already exist.";

					CarbonUIMessage.sendCarbonUIMessage(message,
							CarbonUIMessage.INFO, request, response,
							getContextRoot(request) + "/" + webContext
									+ "/service-mgt/index.jsp");
					log.error(message);
					return false;
				} else {
					message = "Services with the names "
							+ stringBuilder.toString() + " already exist";
					CarbonUIMessage.sendCarbonUIMessage(message,
							CarbonUIMessage.ERROR, request, response,
							getContextRoot(request) + "/" + webContext
									+ "/js_service/upload.jsp");
			...
			JSServiceUploadData[] services = jsServiceDataList
					.toArray(new JSServiceUploadData[jsServiceDataList.size()]);
			serviceUploaderClient.uploadService(username, services);

			message = "Javascript Services uploaded successfully.";
			log.info(message);
			CarbonUIMessage.sendCarbonUIMessage(message, CarbonUIMessage.INFO,
					request, response, getContextRoot(request) + "/"
